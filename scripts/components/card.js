...
export default class cards {
  ...
  static displayCards = (recipes) => {
    ...
    recipes.reverse().forEach((recipe) => {
      cardsContainer.insertAdjacentHTML('afterbegin', `<div id="${recipe.id}" data-ustensils="${[recipe.ustensils]}" data-appliance="${recipe.appliance}" data-ingredients="${recipe.ingredients.map((ingredient) => ingredient.ingredient.toLowerCase())}" class="card flex flex-col w-96 h-96">
